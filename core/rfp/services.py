...
class RFPProcessor:
    ...
    def process(self) -> int:
        ...
        """
        Full pipeline. Returns the number of requirements extracted.
        """
    ...
    def _extract_large_document(self, text:str) -> list:
        ...
        for i, section in enumerate(sections):
            ...
            result = self._extract_requirements(section)
            for req in result.requirements:
                req.position += position_offset
            all_requirements.extend(result.requirements)
            position_offset += len(result.requirements)
        
        return self._deduplicate(all_requirements)

    def _deduplicate(self, requirements: list) -> list:
        seen_text = []
        unique = []
        for req in requirements:
            text_lower = req.requirement_text.lower().strip()
            is_duplicate = False
        
            for seen in seen_text:
                shorter = min(len(text_lower), len(seen))
                if shorter > 0 : 
                    overlap_char = sum(
                        a==b for a,b in zip(text_lower[:shorter], seen[:shorter])
                    )
                    similarity = overlap_char/shorter
                    if similarity > 0.8:
                        is_duplicate=True
                        break
            if not is_duplicate:
                seen_text.append(text_lower)
                unique.append(req)
        
        logger.info(f"Deduplicated: {len(requirements)} -> {len(unique)} requirements")
    ...
    def _save_requirements(self, requirements: list):
        ...
        Requirement.objects.bulk_create(db_requirements)
        logger.info(f"Saved {len(db_requirements)} requirements to DB")
